...
3. 有多个默认参数时，调用的时候，既可以按顺序提供默认参数，也可以不按顺序提供部分默认参数。当不按顺序提供部分默认参数时，需要把参数名写上并赋值，其他默认参数仍继续使用默认值。

4.  定义默认参数要牢记一点：**默认参数必须指向不变对象！**，否则，使用默认参数多次调用时并不会返回预期结果，如：
...
**注意：**如果已经有一个list或者tuple，Python允许你在list或tuple前面加一个**`*`**号，把list或tuple的元素变成可变参数传进去：
