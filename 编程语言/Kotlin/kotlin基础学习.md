...
>  「泛型」，它的意思是把具体的类型泛化，<font color=red>编码的时候用符号来指代类型，在使用的时候（运行时），再确定它的类型</font>。编码时确定类型，降低类型转换错误的发生。
>
>  
>
>  因此，这也导致如果使用反射获取泛型的具体类型，反射的对象必须为声明泛型类型的类具体实例才可以：
>
>  ```java
>  Class TestA<T>{
>  	T a;
...
>  	TestB(T t){
>          b = t;
>          TestA<T> testA = new TestA<T>();
>          testA.a = b;
>      }
>  }
>  
>  TestB<String> testB = TestB<String>("Hello");
>  ```
>
>  这个时候如果在 TestA 中通过反射获取 T 的具体类型是会失败的，因为这是 Java 泛型擦除的限制，在运行时不存在类型传递的机制。
